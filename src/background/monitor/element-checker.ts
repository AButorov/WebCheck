...
// Интерфейс для ответа от offscreen-документа
...
export async function checkElement(
  url: string,
  selector: string,
  maxRetries = CHECK_CONFIG.DEFAULT_MAX_RETRIES
): Promise<CheckResult> {
  console.log(
    `[ELEMENT CHECKER] Checking element at ${url} with selector: ${selector} (via offscreen)`
  )
 ...
 */
export async function checkElementViaTabs(
  url: string,
  selector: string,
  maxRetries = 3
): Promise<CheckResult> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      console.log(
        `[ELEMENT CHECKER] Created tab ${tab.id} for checking (attempt ${attempt}/${maxRetries})`
      )
