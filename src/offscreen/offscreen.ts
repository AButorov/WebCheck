...
const activeIframes = new Map<string, IframeData>()
let iframeCounter = 0
 ...
 */
browser.runtime.onMessage.addListener((message: OffscreenMessage, sender: unknown, sendResponse: (response?: unknown) => void) => {
 ...
 */
async function handleUrlProcessing(message: OffscreenMessage, sendResponse: (response?: unknown) => void): Promise<void> {
  ...
  while (attempt < OFFSCREEN_CONFIG.MAX_RETRY_ATTEMPTS) {
    try {
      ...
      // Извлекаем контент
      const content = await extractContentFromIframe(iframe, selector, requestId || 'unknown')
 ...
 */
function createIframe(url: string, requestId: string): Promise<HTMLIFrameElement> {
 ...
 */
function extractFromDocument(document: Document, selector: string, resolve: (value: string) => void, reject: (reason: Error) => void): void {
  try {
 ...
 * Очистка iframe
 */
function cleanupIframe(requestId: string): void {
 ...
 */
function updateStats(processingTime: number, success: boolean): void {
  if (success) {
