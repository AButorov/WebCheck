...
function activateElementSelector() {
  ...
  instructions.innerHTML =
    'Кликните на элемент для отслеживания. <button id="cancel-selection">Отмена</button>'
  ...
  function cleanup() {
    ...
    try {
      ...
      // Пробуем отправить через webext-bridge как запасной вариант
      sendMessage('element-selection-cancelled', null, { context: 'popup', tabId: -1 }).catch(
        console.error
      )
    }
  }
...
browser.runtime.onMessage.addListener(
  (
    ...
    if (message.action === 'activateElementPicker') {
      console.log(
        '[WebCheck:ContentScript] Received activateElementPicker message, redirecting to element-picker'
      )
      ...
      if (Object.prototype.hasOwnProperty.call(window, 'initElementPicker')) {
